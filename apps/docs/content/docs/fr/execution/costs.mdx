...
```bash
curl -X GET -H "X-API-Key: YOUR_API_KEY" -H "Content-Type: application/json" https://peerbie.com/api/users/me/usage-limits
```
...
- Examinez votre utilisation actuelle dans [Paramètres → Abonnement](https://peerbie.com/settings/subscription)
...
- Consultez votre utilisation actuelle dans [Paramètres → Abonnement](https://peerbie.com/settings/subscription)
